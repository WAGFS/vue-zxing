...
export default {
  ...
  props:{
    parseType:{
      ...
      validator: function (value) {
        ...
        if(!result) throw new Error("parseType can only be 'Camera' or 'Image'")
        return true
      }
    },
  ...
  created() {
    this.parseType === 'Camera' && this.openScan()
    let _this = this
    this.reader.onload = function(e) {
      if(!e.target.result) throw new Error('[FileReader]:Image parsing failed')
  ...
  mounted() {
    // 防止获取不到父组件中的元素
      ...
      this.inputDom && this.inputDom.addEventListener('change',this.selectImg(this),false)
    })
  },
  methods: {
    async openScan() {
    ...
    async parseStaticImg(imgSrc){
      this.codeReader = await new BrowserMultiFormatReader()
      let res = this.codeReader.decodeFromImage(undefined, imgSrc).then((result) => {
